#!/usr/bin/env python3
"""Independent b13 C0a_goal E01 episodes with matched frozen-teacher references."""
...
SOURCE=BASE/'runs/c0a_goal_b13_continuation_20260921/evaluation_seed_selection.json'
...
def run_episode(args,manifest,job):
    ...
    item['candidate_roles']=['student_b13_greedy','teacher_reference']
    torch.save(item,out/'branch_input.pt');request['capture_path']=str(out/'branch_input.pt')
    write_json(out/'decision.json',{'checkpoint_sha256':args.checkpoint_sha,'student_slot':slot,'student_target_id':int(item['candidate_target_ids'][0]),'teacher_target_id':job['teacher_target_id'],'legal_slots':valid.nonzero().flatten().tolist(),'source':'fresh_b13_logits_argmax_before_any_branch_return'})
...
def summarize(rows,protocol):
    ...
    return {'status':'complete','candidate':'b13','stage':'C0a_goal','student_episodes':32,'teacher_reference_branches':32,
...
def main():
    ...
    args.output_dir=args.output_dir.resolve();args.checkpoint=BASE/'runs/c0a_goal_b13_continuation_20260921/training/checkpoints/working.pt';args.checkpoint_sha=file_sha256(args.checkpoint)
    if args.checkpoint_sha!='047a3058060d5e1a6c2a182e6204e0350c2b9b12057ce69bff899f0e523aeb1f':raise ValueError('unexpected b13 checkpoint')
    ...
    train_protocol_path=BASE/'runs/c0a_goal_b13_continuation_20260921/training/protocol.json'
    train_protocol=json.loads(train_protocol_path.read_text())
    data_path=Path(train_protocol['candidate_batch'])
    if file_sha256(data_path)!=train_protocol['data_sha256']:raise ValueError('b13 actual training data hash mismatch')
    ...
    if sorted(training_seeds)!=train_protocol['validation']['training_seeds']:raise ValueError('b13 actual training seed list mismatch')
    training_summary_path=train_protocol_path.parent/'training_summary.json'
    if json.loads(training_summary_path.read_text())['output_sha256']!=args.checkpoint_sha:raise ValueError('b13 training/evaluation checkpoint mismatch')
    previous_protocol_path=BASE/'validations/b12_e01_new32_20260921/protocol.json'
    previous_protocol=json.loads(previous_protocol_path.read_text())
    excluded_previous_seeds=sorted(set(previous_protocol['seeds'])|set(previous_protocol['excluded_b11_evaluation_seeds']))
    if source['excluded_previous_evaluation_seeds']!=excluded_previous_seeds:raise ValueError('excluded prior64 seed set mismatch')
    ...
    if file_sha256(Path(train_protocol['source_checkpoint']))!=train_protocol['source_sha256'] or train_protocol['source_sha256']!=previous_protocol['checkpoint_sha256']:raise ValueError('b13 must continue evaluated b12')
    ...
    if set(seeds)&set(excluded_previous_seeds):raise ValueError('b13 evaluation seeds overlap prior64')
    jobs=[]
    ...
    protocol={'capture_runtime':'evaluation-only, allows SEARCH exactly when original legal mask allows it; rejects empty slots; no mask modifications','reuse_run':str(args.reuse_run) if args.reuse_run else None,'candidate':'b13','student_episodes':32,'teacher_reference_branches':32,'checkpoint':str(args.checkpoint),'checkpoint_sha256':args.checkpoint_sha,
              ...
              'seed_selection':'sort existing manifest by seed; exclude prior64 b10/b11/b12 evaluation seeds; take first32 with original assigned targets; no score filtering','seeds':seeds,'training_seeds':sorted(training_seeds),
              'workers':args.workers,'branch_workers':2,'max_steps':3000,'student_action':'fresh b13 legal argmax at goal boundary; exactly one decision per episode',
    ...
    protocol['previous_b12_protocol']=str(previous_protocol_path)
    protocol['previous_b12_protocol_sha256']=file_sha256(previous_protocol_path)
    ...
    protocol['cross_model_mean_comparison']='different evaluation seeds; do not interpret b13 minus prior b11/b12 means as paired model improvement'
    ...
    try:
        ...
        report=['# b13 E01 独立 32 种子评估','',f"b13 回合：32；同种子教师参考分支：32。平均分：{result['score_mean']:.6f}，标准差：{result['score_std']:.6f}。",'',f"本次 32 回合数值门槛：{result['course_numeric_gates_decision']}。正式晋级：未满足原计划 128 配对回合的样本量要求。",'','| 门槛 | 结果 |','|---|---|']
        report.extend(f"| {k} | {'PASS' if v['pass'] else 'FAIL'} |" for k,v in result['gates'].items())
        report.extend(['','全部 b13 动作由当前检查点重新推理，未读取历史 b9 动作或回报；教师参考也重新仿真。','每个种子只接管一个目标选择边界，其他动作及之后闭环控制由冻结教师执行；原始合法集合包含 SEARCH 时保留该动作，选择 SEARCH 后立即交还教师。','本轮 32 个种子与 b13/b12/b11/b10/b9 本轮训练池及此前 b10/b11/b12 的 64 个评估种子分离，但来自既有课程验证池，不能视为从未用于历史模型评估的新测试集。','',f"检查点 SHA256：{args.checkpoint_sha}"])
