#!/usr/bin/env python3
"""Report b12 training and new32 E01 gates, including conditional mean-score cutoffs."""
import json
from datetime import datetime,timezone
from pathlib import Path
from evaluate_c0a_goal_b12_e01 import BASE,file_sha256,summarize,write_json
...
def main():
    ...
    train=BASE/'runs/c0a_goal_b12_continuation_20260921/training'
    ...
    lines=['# b12：续训及不同 32 种子 E01 课程评估','',f'生成时间：{datetime.now(timezone.utc).isoformat()}。','',
           f'**b12 平均得分 {summary["score_mean"]:.6f}，标准差 {summary["score_std"]:.6f}；本次课程数值判定 {summary["course_numeric_gates_decision"]}。**','',
           '## 续训结果','',
           '从已评估的 b11 检查点继续 48 次全批更新，沿用 64 个训练状态、960 个完整合法候选回报。固定 b9 锚点、λ=0.01、新 Adam 和固定学习率 2e-5，与 b11 续训规则相同。',
           ...
           '## 32 回合评估结果','',
           '32 个学生回合及 32 个教师参考分支全部重新仿真，使用历史固定清单第 33–64 位，与 b10/b11 上轮第 1–32 位及实际训练种子无交集。',
    ...
    lines+=['','SEARCH-only 回合没有目标选择空间，不能单独作为目标选优能力的证据。','',
            '## 逐种子原始得分','', '| seed | b12 目标 | 教师目标 | b12 得分 | 教师得分 | 配对分差 |','|---:|---:|---:|---:|---:|---:|']
    for r in rows:
    ...
    lines+=['','目标 −100 表示 SEARCH。','', '## 范围与复核','',
            ...
            '- 种子与训练及上轮评估分离，但来自历史验证池，不是从未用于任何历史模型的新测试集。',
            ...
            f'- b12 检查点：`{protocol["checkpoint"]}`。',f'- 检查点 SHA256：`{protocol["checkpoint_sha256"]}`。','']
    ...
    if recovery_path.exists():
        ...
        lines+=['## 中断恢复记录','',
                ...
                '中断前的未完成分支不计入得分；最终仍为 32 个唯一、完整的 b12 学生回合及 32 个教师参考。未按中途成绩筛选、替换种子或改变检查点。详见 INTERRUPTION_RECOVERY.json 与 interrupted_attempts/。','']
    (directory/'RESULTS.md').write_text('\n'.join(lines))
    write_json(BASE/'runs/c0a_goal_b12_continuation_20260921/completion.json',{'status':'complete','training_summary':str(train/'training_summary.json'),'evaluation_summary':str(directory/'evaluation_summary.json'),'report':str(directory/'RESULTS.md'),'course_numeric_gates_decision':summary['course_numeric_gates_decision'],'formal_promotion':False})
