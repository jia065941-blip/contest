#!/usr/bin/env python3
"""Independent b11 C0a_goal E01 episodes with matched frozen-teacher references."""
...
def run_episode(args,manifest,job):
    ...
    item['candidate_roles']=['student_b11_greedy','teacher_reference']
    torch.save(item,out/'branch_input.pt');request['capture_path']=str(out/'branch_input.pt')
    write_json(out/'decision.json',{'checkpoint_sha256':args.checkpoint_sha,'student_slot':slot,'student_target_id':int(item['candidate_target_ids'][0]),'teacher_target_id':job['teacher_target_id'],'legal_slots':valid.nonzero().flatten().tolist(),'source':'fresh_b11_logits_argmax_before_any_branch_return'})
...
def summarize(rows,protocol):
    ...
    return {'status':'complete','candidate':'b11','stage':'C0a_goal','student_episodes':32,'teacher_reference_branches':32,
...
def main():
    ...
    args.output_dir=args.output_dir.resolve();args.checkpoint=BASE/'runs/c0a_goal_b11_diagnostics_20260921/b11/checkpoints/working.pt';args.checkpoint_sha=file_sha256(args.checkpoint)
    if args.checkpoint_sha!='6401c73ba7679cb8548cb30c2e47e10f7c291093a5afdc95a4b8db179bb774a4':raise ValueError('unexpected b11 checkpoint')
    ...
    protocol={'capture_runtime':'evaluation-only, allows SEARCH exactly when original legal mask allows it; rejects empty slots; no mask modifications','reuse_run':str(args.reuse_run) if args.reuse_run else None,'candidate':'b11','student_episodes':32,'teacher_reference_branches':32,'checkpoint':str(args.checkpoint),'checkpoint_sha256':args.checkpoint_sha,
              ...
              'seed_selection':'first 32 in pre-existing b9 evaluation order; same preselected seeds as b10; no b11 results used','seeds':seeds,'training_seeds':sorted(training_seeds),
              'workers':args.workers,'branch_workers':2,'max_steps':3000,'student_action':'fresh b11 legal argmax at goal boundary; exactly one decision per episode',
    ...
    comparison_keys=['teacher_sha256','scenario_sha256','manifest_sha256','seed_boundary_source_sha256','seeds','training_seeds','jobs','trace_sha256','max_steps','continuation','score_threshold','fixed_score_threshold','suffix_ratio_min','confidence','formal_promotion_min_episodes']
    ...
    try:
        ...
        report=['# b11 E01 独立 32 种子评估','',f"b11 回合：32；同种子教师参考分支：32。平均分：{result['score_mean']:.6f}，标准差：{result['score_std']:.6f}。",'',f"本次 32 回合数值门槛：{result['course_numeric_gates_decision']}。正式晋级：未满足原计划 128 配对回合的样本量要求。",'','| 门槛 | 结果 |','|---|---|']
        report.extend(f"| {k} | {'PASS' if v['pass'] else 'FAIL'} |" for k,v in result['gates'].items())
        report.extend(['','全部 b11 动作由当前检查点重新推理，未读取历史 b9 动作或回报；教师参考也重新仿真。','每个种子只接管一个目标选择边界，其他动作及之后闭环控制由冻结教师执行；原始合法集合包含 SEARCH 时保留该动作，选择 SEARCH 后立即交还教师。','种子与 b11/b10/b9 本轮训练池分离，但来自既有课程验证池，不能视为从未用于历史模型评估的新测试集。','',f"检查点 SHA256：{args.checkpoint_sha}"])
