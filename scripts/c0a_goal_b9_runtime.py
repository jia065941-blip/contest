"""Simulation-side b9 state capture and symmetric closed-loop teacher branches.
...
def run_request(env, commander, learning_policy, run_summary, args,
    ...
    if request["mode"] == "capture":
        ...
        env.prepare_native_handoff_actions(boundary_row["actions"], [unit], "goal")
        sample = learning_policy._pending_motion[executor]
        valid = sample.target_valid_mask.detach().cpu().bool()
        features = sample.target_features.detach().cpu().unsqueeze(0)
        encoded = sample.observation.detach().cpu().unsqueeze(0)
        with torch.no_grad():
            logits = learning_policy.trainer.model.distribution_parameters(
                encoded, target_features=features,
                target_valid_mask=valid.unsqueeze(0))["target_logits"][0].cpu()
        for index in valid.nonzero().flatten().tolist():
            if learning_policy._target_slot_ids[index] in (None, -100):
                raise RuntimeError("goal legal mask includes empty/search slot")
        generator = torch.Generator().manual_seed(int(request["sampling_seed"]))
        indices, roles = sample_candidates(logits, valid, request["k"],
                                           request["rho"], generator)
        item = {
            "seed": request["seed"], "start_state_id": request["start_state_id"],
            "boundary_source": request["boundary_source"], "timestep": boundary,
            "executor_id": executor, "observation": encoded,
            "target_features": features, "target_valid_mask": valid.unsqueeze(0),
            "candidate_target_indices": torch.tensor(indices, dtype=torch.long),
            "candidate_target_ids": torch.tensor([
                learning_policy._target_slot_ids[i] for i in indices]),
            "candidate_coordinates": sample.target_coordinates[indices].detach().cpu(),
            "candidate_roles": roles, "old_logits": logits,
            "snapshot_model_sha256": request["snapshot_model_sha256"],
            "physical_state_sha256": physical_sha,
            "sampling_seed": request["sampling_seed"],
        }
        torch.save(item, capture_path)
        save_json(capture_path.with_suffix(".json"), {
            key: value.tolist() if isinstance(value, torch.Tensor) else value
            for key, value in item.items() if key not in
            {"observation", "target_features", "old_logits", "target_valid_mask"}})
        return
            ...
            with (result_dir / f"branch_{candidate_index:02d}.log").open("w") as log:
                ...
                for step in range(boundary, int(args.max_steps) + 1):
                    if step == boundary:
                        ...
                        coordinates = item["candidate_coordinates"][candidate_index]
                        for action in actions:
                            if (int(action.get("executor_id", -1)) == executor and
                                    int(action.get("commandType_id", -1)) == 200):
                                original = copy.deepcopy(action)
                                action["target"] = {"x": float(coordinates[0]),
                                                    "y": float(coordinates[1]), "z": 0.}
                                non_target_boundary_changes += int(
                        ...
                        if locked:
                            commander.target_by_platform[executor] = int(target)
                    if identity_audit and step == boundary and actions != boundary_row["actions"]:
                ...
                if identity_audit:
                    audit_fields = {
                        ...
                        "exact_boundary_actions_equal": True,
                    }
