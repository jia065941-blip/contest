...
def main():
    ...
    protocol={'capture_runtime':'evaluation-only, allows SEARCH exactly when original legal mask allows it; rejects empty slots; no mask modifications','reuse_run':str(args.reuse_run) if args.reuse_run else None,'candidate':'b10','student_episodes':32,'teacher_reference_branches':32,'checkpoint':str(args.checkpoint),'checkpoint_sha256':args.checkpoint_sha,
    ...
    try:
        ...
        report.extend(['','全部 b10 动作由当前检查点重新推理，未读取历史 b9 动作或回报；教师参考也重新仿真。','每个种子只接管一个目标选择边界，其他动作及之后闭环控制由冻结教师执行；原始合法集合包含 SEARCH 时保留该动作，选择 SEARCH 后立即交还教师。','种子与 b10/b9 本轮训练池分离，但来自既有课程验证池，不能视为从未用于历史模型评估的新测试集。','',f"检查点 SHA256：{args.checkpoint_sha}"])
